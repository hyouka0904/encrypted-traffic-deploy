...
encrypted-traffic-deploy/
├── portal/
│   ├── app.py                 # Flask Captive Portal（含實驗 start/stop/abort endpoint）
│   ├── portal.db              # SQLite（自動建立）
│   ├── setup_iptables.sh      # iptables 攔截規則初始化
│   ├── static/
│   │   ├── experiment.js      # 實驗流量產生腳本（瀏覽器執行）
│   │   └── testfile.bin       # FT 流量用大檔案（不推 git，自行產生）
│   └── templates/
│       ├── portal.html
│       ├── login.html
│       ├── register.html
...
每個 stable tick append 一筆到 `logs/flow_infer_log.csv`，格式 `timestamp,ip,label,model`。此檔以 **append 模式**持續累積、跨實驗不覆蓋、不推 git，作為 Portal 截取實驗區間的原始資料來源。`model` 欄位記錄當前 flow_monitor 載入的模型（rf / xgb / lgb），因為模型在 flow_monitor 啟動時就固定、無法中途更換，Portal 截取時直接從此欄得知是哪個模型。

`--log` 參數已移除，log 路徑固定，不可自訂。
...
各 client IP 獨立維護實驗 session，互不干擾，支援多人同時跑實驗（適合 demo 現場多台裝置同時操作）。
...
實驗時 client 需確保流量只走 AP（wlan1），否則 flow_monitor 抓不到。

**推薦：手機**。手機只有一張無線網卡，連上 `my_rpi_AP` 後流量天然只走這條路徑，最簡單可靠。

**筆電（備選）**：需在實驗期間停用或拔除其他網路介面（有線網卡、其他 WiFi），確保只有 AP 這條介面 active，否則流量可能走其他介面而被 flow_monitor 漏抓。
...
- **FT**：持續 fetch 大檔案 `/static/testfile.bin`（由 app.py 提供），完成後立刻再發，模擬檔案傳輸的持續大流量
- **P2P**：每 8 秒取一次 `testfile.bin` 前 32KB（Range header），模擬 P2P 的小量持續流量

### status 頁面實驗按鈕行為

- 按下「**Start Experiment**」後按鈕變為紅色「**Stop Experiment**」
- 手動點「Stop Experiment」：走 `POST /experiment/abort`，清除所有 timer，**不產生任何 log 紀錄**（視為中斷，不計入評估）
- 實驗自然跑完（510 秒）：自動走 `POST /experiment/stop`，頁面顯示完成提示，包含模型名稱與 tick 數

### 實驗相關 Portal 路由

| 路由 | 說明 |
|------|------|
| `POST /experiment/start` | 記下開始時間與 client IP，建立該 IP 的實驗 session |
| `POST /experiment/stop` | 從 `flow_infer_log.csv` 截取區間內該 IP 的紀錄，換算 offset，存成 `logs/infer_<model>_<timestamp>.csv` |
| `POST /experiment/abort` | 清除該 IP 的實驗 session，不讀 log、不存檔 |
...
> **前置條件**（需先完成）：
> - [安裝套件](#安裝套件) — `pip install -r requirements.txt`
> - [環境變數設定](#環境變數設定) — `FLASK_SECRET_KEY`、`ADMIN_PASSWORD_HASH`
> - [Raspberry Pi AP 設定](#raspberry-pi-ap-設定) — hostapd、dnsmasq 已設好並開機自啟
> - [模型部署](#模型部署models) — `models/` 已放好 ONNX 檔案
> - 測試大檔案已產生 — `head -c 100M /dev/urandom > portal/static/testfile.bin`

三個模型（rf / xgb / lgb）各跑一輪，每輪步驟如下：
...
source venv/bin/activate
sudo venv/bin/python inference/flow_monitor.py --iface wlan1 --model rf
# 下一輪換 --model xgb，再下一輪換 --model lgb
```
...
```
4. 連上 my_rpi_AP（無密碼）
5. 通過 Captive Portal 登入（若尚未有帳號請先註冊，等 Admin 在 Dashboard 審核）
6. 進入 Status 頁面，按「Start Experiment」
7. 等待 510 秒，頁面自動顯示完成提示（含 tick 數）
   （如需中止請按「Stop Experiment」，本輪不計入評估）
```

**【Pi 端】結束本輪**

```bash
# 8. 停止 flow_monitor：Ctrl+C
# 9. 清除 QoS 規則
sudo venv/bin/python inference/qos_controller.py --clear
# 10. 停止 Portal：Ctrl+C
```
...
三輪實驗跑完後，對每份 infer log 執行 eval：

```bash
python experiment/eval.py --infer logs/infer_rf_<timestamp>.csv
python experiment/eval.py --infer logs/infer_xgb_<timestamp>.csv
python experiment/eval.py --infer logs/infer_lgb_<timestamp>.csv
```

- `--infer`：指定要評估的 infer log
- `--gt`：預設讀 `experiment/ground_truth.csv`，不需指定
- 輸出：自動產生 `results/result_<model>_<timestamp>.json`（model 與 timestamp 從 infer log 檔名解析）

result JSON 內含三部分：
1. 準確度統計（overall accuracy + per-label accuracy + predicted_as）
2. 逐筆推論明細（offset_sec, ip, predicted, ground_truth, correct）
3. 使用的 ground truth 序列

terminal 同時印出 overall accuracy 與 per-label 明細，方便現場確認。

**對齊邏輯**：ground_truth 定義時間區間（offset 0\~45 為 BROWSING，45\~115 為 FT，依此類推）。infer log 每筆推論的 offset_sec 落在哪個區間，就以該區間的 label 為 ground truth 比對。推論筆數不需與 ground truth 段數相同，每筆獨立判斷。
...
- [ ] 新增一個腳本或者程式 讓使用者只要用它就可以完成上面繁瑣的設定