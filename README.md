...
每 10 個 tick 印一次統計摘要，包含：

- **推論耗時**：每次 `session.run` 的耗時（mean / max / std，單位 ms）
- **CPU / RAM**：process 當下的 CPU 使用率與記憶體用量
- **label 分布**：各 class 被預測的次數與百分比
- **準確度**：需搭配 `--ground-truth` 參數使用（見下方）

### 指定模型與 ground-truth

`--model` 參數指定模型名稱（不含 `.onnx`），找不到時列出 `models/` 內可用清單。

`--ground-truth` 參數指定當前實驗流量的真實類別，flow_monitor 會將每個 stable tick 的推論結果與之比對，在 summary 裡輸出準確度。**使用方式**：在電腦端跑已知類別的流量（例如用 iperf3 模擬 FT），同時在 Pi 端啟動 flow_monitor 並帶上對應的 `--ground-truth` 值，跑完後看 summary 的準確度欄位。
...
# 量準確度（電腦端正在跑 FT 流量時）
sudo venv/bin/python inference/flow_monitor.py --iface wlan1 --model rf --ground-truth FT
...
**QoS 限速驗證**
- [ ] iperf3 實際驗證下行限速是否生效：Pi 開 `iperf3 -s`，client 打流量，`tc -s class show dev wlan1` 確認流量進對應 class

**流量實驗**
- [ ] 設定實驗網路環境：ThinkPad 第二張 Edimax 連 `my_rpi_AP` 跑實驗流量，SSH 走 Tailscale，確保兩條路分開不互相污染
- [ ] RF / XGB / LGB 三模型各跑一輪已知流量，收集 summary（推論耗時、CPU/RAM、label 分布）
- [ ] 加 `--ground-truth` 量測三模型準確度
